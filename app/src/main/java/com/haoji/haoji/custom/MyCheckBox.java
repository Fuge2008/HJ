...
import com.haoji.haoji.util.SizeUtils;
...
public class MyCheckBox extends View implements Checkable {
	...
	private static final int COLOR_FLOOR_UNCHECKED = Color
			.parseColor("#DFDFDF");
	...
	private void init(AttributeSet attrs) {
		...
		int tickColor = ta.getColor(R.styleable.MyCheckBox_color_tick,
				COLOR_TICK);
		mAnimDuration = ta.getInt(R.styleable.MyCheckBox_duration,
				DEF_ANIM_DURATION);
		mFloorColor = ta.getColor(
				R.styleable.MyCheckBox_color_unchecked_stroke,
				COLOR_FLOOR_UNCHECKED);
		mCheckedColor = ta.getColor(R.styleable.MyCheckBox_color_checked,
				COLOR_CHECKED);
		mUnCheckedColor = ta.getColor(
				R.styleable.MyCheckBox_color_unchecked, COLOR_UNCHECKED);
		mStrokeWidth = ta.getDimensionPixelSize(
				R.styleable.MyCheckBox_stroke_width,
				SizeUtils.dp2px(getContext(), 0));
		ta.recycle();
	...
	private void reset() {
		...
		mDrewDistance = isChecked() ? (mLeftLineDistance + mRightLineDistance)
				: 0;
	}
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		mStrokeWidth = (mStrokeWidth == 0 ? getMeasuredWidth() / 10
				: mStrokeWidth);
		...
		mLeftLineDistance = (float) Math.sqrt(Math.pow(mTickPoints[1].x
				- mTickPoints[0].x, 2)
				+ Math.pow(mTickPoints[1].y - mTickPoints[0].y, 2));
		mRightLineDistance = (float) Math.sqrt(Math.pow(mTickPoints[2].x
				- mTickPoints[1].x, 2)
	...
	private void drawBorder(Canvas canvas) {
		...
		canvas.drawCircle(mCenterPoint.x, mCenterPoint.y, radius * mFloorScale,
				mFloorPaint);
	}
	...
	private void drawTickPath(Canvas canvas) {
		...
		if (mDrewDistance < mLeftLineDistance) {
			...
			float stopX = mTickPoints[0].x
					+ (mTickPoints[1].x - mTickPoints[0].x) * mDrewDistance
					/ mLeftLineDistance;
			float stopY = mTickPoints[0].y
					+ (mTickPoints[1].y - mTickPoints[0].y) * mDrewDistance
					/ mLeftLineDistance;
	...
	private void startCheckedAnimation() {
		...
		animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				...
				mFloorColor = getGradientColor(mUnCheckedColor, mCheckedColor,
						1 - mScaleVal);
				postInvalidate();
			}
		});
		...
		animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				...
				mFloorColor = getGradientColor(mCheckedColor,
						mFloorUnCheckedColor, mScaleVal);
				postInvalidate();
			}
		});
		animator.start();
