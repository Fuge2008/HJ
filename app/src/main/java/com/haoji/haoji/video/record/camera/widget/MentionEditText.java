 ...
 * @author Andy
 *
 * Created by lky on 2017/6/12.
 */
public class MentionEditText extends MaxLengthEditText {
    ...
    protected void onSelectionChanged(int selStart, int selEnd) {
        ...
        if (selStart == selEnd) {
            mSelectedStart = nearbyRange.getAnchorPosition(selStart);
            try{
                setSelection(mSelectedStart);
    ...
    public void insertText(String insertStr){
        getText().insert(mSelectedStart,insertStr);
    }
    ...
    private void init() {
        ...
        mMentionTextColor = 0xFFFFD217;
        //disable suggestion
//        setInputType(InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
        addTextChangedListener(new MentionTextWatcher());
        if(!isCanInputLineFeed){
...
//                } else {
                    ...
                    editText.getText().delete(closestRange.from,closestRange.to);
