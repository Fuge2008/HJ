...
public class OkHttpClientManager {
    ...
    private Gson mGson;
    private OkhttpDownloadListener downloadListener;
    ...
    private void _getAsyn(String url, final ResultCallback callback, Param... params) {
        ...
        for (Param param : params) {
            if (i > 0)
                tempParams.append("&");
            tempParams.append(String.format("%s=%s", param.key, param.value));
            i++;
        }
    ...
    private void _downloadFile(final String url, final String destFileDir, final OkhttpDownloadListener downloadListener) {
            ...
            public void onResponse(Response response) {
                ...
                try {
                    ...
                    while ((len = is.read(buf)) != -1) {
                        fos.write(buf, 0, len);
                        downlen += len;
                        double a = ((double) downlen / (double) size);
                        getInstance().downloadListener.download((int) (a * 100));
                    }
                    fos.flush();
                    // 如果下载文件成功，第一个参数为文件的绝对路径
                } catch (IOException e) {
    ...
    public static void downloadFile(String url, String destFileDir, OkhttpDownloadListener downloadListener) {
        getInstance()._downloadFile(url, destFileDir, downloadListener);
    }
    ...
    public OkhttpDownloadListener getDownloadListener() {
        return downloadListener;
    }

    public void setDownloadListener(OkhttpDownloadListener downloadListener) {
        this.downloadListener = downloadListener;
    }

    public interface OkhttpDownloadListener {

        public void download(int cont);
    }
